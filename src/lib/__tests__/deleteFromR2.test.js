...
// deleteFromR2 → freshToken() now returns the caller's FIREBASE ID token.
vi.mock("@/lib/firebase.js", () => ({ firebaseUserToken: async () => "tok" }));
...
describe("deleteFromR2", () => {
  it("sends DELETE to /api/media with the key + Firebase token", async () => {
  ...
  it("throws with the server error message on non-ok response", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({ ok: false, status: 500, json: async () => ({ error: "delete failed" }) });
    await expect(deleteFromR2("bad/key")).rejects.toThrow("delete failed");
  });

  it("tags a 409 in-use rejection with code and usedBy", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({ ok: false, status: 409, json: async () => ({ error: "in_use", usedBy: "demo" }) });
    await expect(deleteFromR2("uuid/owner/image/hero/x-photo.jpg")).rejects.toMatchObject({ code: "in_use", usedBy: "demo" });
  });
});
